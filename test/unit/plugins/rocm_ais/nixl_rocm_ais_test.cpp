...
#define TEST_PHRASE "NIXL hipFile Storage Test Pattern 2025"
...
print_usage(const char *program_name) {
    std::cerr << "Usage: " << program_name << " [options] <directory_path>\n"
              ...
              << "  -n, --num-transfers N   Number of transfers (default: " << DEFAULT_NUM_TRANSFERS
              << ")\n"
              << "  -s, --size SIZE         Size of each transfer (default: "
              << DEFAULT_TRANSFER_SIZE << " bytes)\n"
              << "                          Can use K, M, or G suffix\n"
              << "  -r, --no-read           Skip read test\n"
              << "  -w, --no-write          Skip write test\n"
              << "  -p, --pool-size SIZE    Batch pool size (default: 8)\n"
              << "  -b, --batch-limit SIZE  Max requests per batch (default: 128)\n"
              << "  -m, --max-req-size SIZE Max size per request (default: 16M)\n"
              << "  -t, --iterations N      Iterations per transfer (default: "
              << DEFAULT_ITERATIONS << ")\n"
              << "  -D, --direct            Use O_DIRECT\n"
              << "  -h, --help              Show this help message\n"
              << "\nExample:\n"
              << "  " << program_name
              << " -d -n 100 -s 2M -p 16 -b 128 -m 16M -t 5 -D /path/to/dir\n";
}
...
main(int argc, char *argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "dvn:s:rwp:b:m:t:Dh", long_options, NULL)) != -1) {
        ...
        case 'm':
            ...
            if (max_request_size > 16 * 1024 * 1024) {
                std::cerr << "Error: Max request size cannot exceed 16M\n";
                return 1;
            }
            break;
        case 't':
    ...
    if (use_dram && use_vram) {
        std::cerr << "Error: Cannot specify both DRAM (-d) and VRAM (-v)\n";
        print_usage(argv[0]);
        return 1;
    }
    ...
    std::cout << "- O_DIRECT: " << (use_direct ? "Yes" : "No") << std::endl;
    std::cout << "- Operation: ";
    if (!skip_read && !skip_write) {
...

    {
        ...
        if (!skip_write) {
            ...
            for (unsigned int iter = 0; iter < iterations; iter++) {
                ...
                if (status < 0) {
                    std::cerr << "Failed to post write transfer" << std::endl;
                    goto cleanup;
                }
        ...
        if (!skip_read) {
            ...
            for (unsigned int iter = 0; iter < iterations; iter++) {
                ...
                if (status < 0) {
                    std::cerr << "Failed to post read transfer" << std::endl;
                    goto cleanup;
                }
