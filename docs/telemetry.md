...
| `agent_ais_mt_` | AIS_MT (AMD) | `agent_ais_mt_read_bytes`, `agent_ais_mt_write_errors` |
...
| `agent_ais_mt_*` | bytes/count | AIS_MT hipFile and HIP metrics (per-GPU where noted) |
| `agent_ais_mt_thread_count` | count | AIS_MT Taskflow executor thread count (gauge) |
